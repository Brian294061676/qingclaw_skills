...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "搭一个反馈收集应用" | 已在轻流创建反馈应用：多渠道收集表单/分类标签/关联需求/处理跟踪，返回链接 |
| "本周反馈有什么新问题？" | 新增问题聚类 + 频次统计 + 需关注的负面反馈预警 |
| "这条反馈处理到哪了？" | 处理链路追踪：当前状态/关联需求/预计上线版本 |
...
```
为反馈应用配置AI自动分类规则，按产品模块和问题类型自动打标。
```

### 指令4：闭环通知配置

```
配置反馈闭环通知：关联需求上线后自动向反馈用户发送处理结果。
```
