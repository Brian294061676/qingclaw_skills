...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "生成本月经营月报" | 经营月报：核心指标达成 + 同环比分析 + 亮点与风险 + 下月重点事项 |
| "对比各业务线表现" | 业务线对比表：收入/利润/增速排名 + 结构变化解读 |
| "上次复盘的改进项进展如何？" | 改进事项跟踪表：状态/责任人/延期项预警 |
...
```
扫描本周经营数据，识别偏离月度目标进度超10%的指标并分析原因。
```

### 指令4：管理层汇报材料

```
把本月经营数据浓缩成一页管理层汇报要点，结论前置、数据佐证。
```
