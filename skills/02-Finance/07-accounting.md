...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "生成本月财务分析报告" | 月度报告：收入/成本/费用核心指标 + 环比同比差异 + 风险预警 + 管理建议 |
| "毛利率为什么下降了？" | 差异拆解：价格/成本/结构三因素影响量化 + 根因说明 |
| "汇总各分公司数据" | 多表归集结果 + 口径一致性检查说明 + 合并指标表 |
...
```
分析本月管理费用明细，找出环比增幅超过20%的科目并说明原因。
```

### 指令3：现金流预警

```
基于应收应付和预算数据，评估未来三个月现金流缺口风险。
```
