...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "识别这批发票"（附图片） | OCR结构化结果表：发票代码/号码/金额/税率/开票方 + 识别置信度标注 |
| "这些进项票和采购单匹配一下" | 匹配结果：已匹配/金额不符/无对应单据三类清单 + 差异说明 |
| "发票入台账" | 已通过 record_insert 写入发票台账，重复发票自动拦截并提示 |
...
```
校验这批发票的税率与商品类别是否匹配，标出疑似错误税率的发票。
```

### 指令3：月度发票汇总

```
按供应商和税率汇总本月进项发票，输出可抵扣税额统计。
```
