...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "搭一个IT工单系统" | 已在轻流创建工单应用：提交表单/AI分类/智能分派/处理跟踪/满意度评价，返回链接 |
| "网络故障工单怎么处理这么慢？" | 该类工单处理时长分析 + 卡点定位 + 分派规则优化建议 |
| "本月IT服务质量如何？" | 服务报告：工单量/按时解决率/满意度/TOP问题类型 |
...
```
高频工单问题自动关联解决方案知识库，提交时先推送自助解决指引。
```

### 指令4：SLA规则配置

```
按工单优先级配置SLA时限和超时升级规则：P1两小时未响应升级到IT主管。
```
...
```
~/.qingclaw/skills/47-it-helpdesk-flow.md
```
