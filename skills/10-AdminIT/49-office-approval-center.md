...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "搭一个行政审批中心" | 已在轻流创建统一审批入口：用车/用章/采购/会议室多场景表单 + 分级审批流，返回链接 |
| "用章审批要加法务会签" | 已在用章流程中加入法务会签节点，合同类用章强制上传附件 |
| "审批效率如何？" | 审批分析：各场景平均时长/超时率/驳回率 + 优化建议 |
...
```
为会议室预订配置冲突检测和自动释放规则：超时15分钟未签到自动释放。
```

### 指令4：审批授权规则

```
配置节假日审批人自动代理规则，避免审批停滞。
```
...
```
~/.qingclaw/skills/49-office-approval-center.md
```
