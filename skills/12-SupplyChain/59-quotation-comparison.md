...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "对比这三家报价" | 比价分析表：价格/账期/交期/质量条款多维对比 + 综合得分 + 推荐结论 |
| "这个价格合理吗？" | 价格评估：历史采购价对比 + 市场参考 + 议价空间建议 |
| "比价结果回写轻流" | 已通过 record_update 回写比价结论和推荐供应商 |
...
```
根据这批采购需求生成标准询价单：规格/数量/交期/商务条款要求。
```

### 指令4：年度采购降本分析

```
汇总本年度比价采购的降本金额，输出降本明细和下年度议价重点。
```
