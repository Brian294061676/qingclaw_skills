...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "分析这个季度的丢单原因" | 丢单原因分类统计 + TOP原因深度分析 + 策略改进建议 |
| "这单为什么赢了？" | 赢单要素拆解：决策链把握/方案匹配/价格策略 + 可复制经验 |
| "对比我们和竞品X的输赢情况" | 对阵记录统计 + 输赢场景规律 + 应对策略建议 |
...
```
分析近一年所有赢单案例的共同特征，提炼可复制的赢单打法清单。
```

### 指令3：丢单挽回评估

```
筛选近90天丢单中仍有挽回可能的客户，输出挽回优先级和切入策略。
```
