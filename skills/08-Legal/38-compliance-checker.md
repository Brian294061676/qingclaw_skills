...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "检查这个制度是否合规" | 合规比对报告：逐条对照法规 + 不合规项 + 整改建议 + 依据条款引用 |
| "新的数据法规对我们有什么影响？" | 影响评估：涉及业务范围 + 差距清单 + 整改优先级 |
| "出一份季度合规报告" | 合规报告：检查覆盖面/发现问题/整改进度/下季度计划 |
...
```
针对个人信息保护合规要求，生成一份业务部门可自查的检查清单。
```

### 指令3：整改跟踪台账

```
将本次合规检查发现的问题建成整改台账，配置到期提醒和复查节点。
```
