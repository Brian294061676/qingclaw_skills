...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "分析产线负荷" | 负荷分析报告：设备/产线利用率 + 瓶颈工序定位 + 不均衡度量化 |
| "瓶颈工序怎么破？" | 优化方案：工艺调整/班组调度/设备加开对比评估 + 预期提升量化 |
| "优化班组排班" | 排班建议：人员-班次-工序匹配方案 + 加班优化建议 |
...
```
基于当前瓶颈分析，输出三档投入的产能提升方案（不投入/小改造/设备扩充）。
```

### 指令4：OEE分析

```
计算各关键设备的OEE（可用率×性能×良率），定位损失最大的环节。
```
