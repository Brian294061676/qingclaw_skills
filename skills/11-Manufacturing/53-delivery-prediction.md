...
| 用户输入 | Agent 输出 |
|---------|-----------|
| "这批订单能按期交付吗？" | 交付风险评估：各订单延期概率 + 风险因素（进度/物料/产能）+ 建议措施 |
| "分析上月延期原因" | 延期归因：原因分类统计 + TOP根因分析 + 改进建议 |
| "风险标记回写轻流" | 已通过 record_update 标记高风险订单并通知相关人 |
...
```
汇总本月订单交付数据：按期率/平均延期天数/风险订单分布。
```

### 指令4：客户沟通建议

```
针对预计延期的订单，生成客户沟通话术和补救方案建议。
```
